...
import yapimath.exceptions.RangeException;
import yapimath.exceptions.YAPIMathException;
import yapimath.range.RangeSimple;
...
public class NumberUtils {
    ...
    public static long factorial(long input) {
        if (input < 0) {
            throw new YAPIMathException("Factorial of negatives is not defined");
        }
        if (input == 0) {
            return 1;
        }
    ...
    public static BigInteger over(BigInteger n, BigInteger r) {
        ...
        return a.divide(b.multiply(factorial(c)));
    }
    ...
    public static boolean isDivisibleByPrimeFactorization(long l, long divisor) {
        long[] longs = toLongArray(primeFactorization(divisor));
        for (long div : longs) {
    ...
    private static long[] toLongArray(List<Long> input) {
